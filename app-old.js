...
app.post('/api/lists/:id', function (req, res) {
  ...
  Item.findOneAndUpdate({_id : req.params.id}, update, function (err, list) {
    if(err){res.json({success: false, error: 'List not updated', success: false});}
  ...
  User.findOne({_id : req.params.id}, function(err, user) {
	...
	var answer = {
	  ...
	  for (var i = 0; i < user.lists.length; i++) {
		  console.log("List ID: "+ user.lists[i]._id);
		  List.findOneAsync({_id : user.lists[i]._id}).then((l) => {
			  answer.lists.push(l);
		  }).catch((err) => {
			  answer.error += err + " ";
		  });
		  console.log("Side Process for List "+user.lists[i]._id+" finished");
		  console.log(answer);
	  }
...
app.post('/api/users/:id', function (req, res) {
  ...
  User.findOneAndUpdate({_id : req.params.id}, update, function (err, user) {
    if(err){res.json({success: false, error: 'User not updated'});}
    res.json(user);
  });
});
